...
------------------------------------------
Semantic diff of two ODCS v3 data contract YAML files.

Public API:
    ContractDiff.generate(v1_path, v2_path)
                              — load, normalize, diff; return plain DeepDiff dict

Report generation is handled by ContractDiffReport in
report_renderer, which delegates to the appropriate renderer:

    HtmlContractDiffRenderer  (html_report_renderer.py)
    TextContractDiffRenderer  (text_report_renderer.py)

Usage:
    python diff.py v1.yaml v2.yaml           # plain-text output
    python diff.py v1.yaml v2.yaml --html    # HTML report

Dependencies:
    pip install deepdiff open-data-contract-standard>=3.1.2
"""
...
class ContractDiff:
    ...
    Usage:
        result = ContractDiff().generate("v1.yaml", "v2.yaml")
    """
    ...
    def _normalize(contract: dict) -> dict:
        ...
        # a single reflection-based loop. (LIST_CONTAINERS in helpers.py already uses
