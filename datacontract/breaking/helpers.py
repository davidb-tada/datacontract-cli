"""
helpers — Shared helpers for ODCS diff renderers
-------------------------------------------------
Provides constants and utility functions consumed by every renderer:

    LIST_CONTAINERS  — frozenset of ODCS container keys whose direct children
                       are named list items and should be rendered with a "- "
                       prefix in diff reports. Derived automatically from the
                       ODCS Pydantic models; no manual maintenance required.
    _flatten_value() — flatten a dict into (label, value) pairs

Imported by:
    report_renderer.py      — ContractDiffReport uses LIST_CONTAINERS
    text_report_renderer.py — TextContractDiffRenderer uses LIST_CONTAINERS and _flatten_value
    html_report_renderer.py — HtmlContractDiffRenderer uses LIST_CONTAINERS
"""
