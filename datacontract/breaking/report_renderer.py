"""
report_renderer — Core report builder and pipeline orchestrator for ODCS diffs
-------------------------------------------------------------------------------
Provides ContractDiffReport, the entry-point class for generating diff reports.

ContractDiffReport.build_report_data() converts a raw ContractDiff.generate() result
into a clean, serialisable dict consumed by every renderer:

    {
      "source_label": str,
      "target_label": str,
      "header":  { "title": str, "subtitle": str, "generated_at": str },
      "summary": { "counts": {...}, "changes": [...] },
      "detail":  { "counts": {...}, "changes": [...] },
    }

summary.changes rolls leaf-level field changes up to their parent path:
  - Scalar Changed (e.g. logicalType: string → date) → parent field
  - Scalar Added/Removed leaves (e.g. businessName added) → parent field
  - Mixed Add+Remove on the same parent → parent with changeType: Changed
  - Dict Added/Removed (whole schema objects) → stay at their own path
  - List item Added/Removed (tags) → rolled up to the tags parent path

detail.changes keeps every leaf change individually, including the tag value
as the final path segment for list scalar items (e.g. tags.pii Removed).

ContractDiffReport.generate() orchestrates the full pipeline:
load → diff → build_report_data → render → output.

Shared helpers (LIST_CONTAINERS, _flatten_value) live in helpers.py;
renderers import directly from helpers.

Renderer modules:
  html_report_renderer.py  — HtmlContractDiffRenderer
  text_report_renderer.py  — TextContractDiffRenderer
"""
...
from datacontract.breaking.diff import ContractDiff
...
class ContractDiffReport:
    ...
    Usage:
        ContractDiffReport().generate(v1_path="v1.yaml", v2_path="v2.yaml", fmt="html")
    """
    ...
    def generate(
        ...
        from datacontract.breaking.text_report_renderer import TextContractDiffRenderer
