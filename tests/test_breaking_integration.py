...
-----------------------------------
Exercises the complete stack end-to-end using the v1.yaml / v2.yaml fixture files:

    v1.yaml / v2.yaml
        → ContractDiff.generate()        (load, normalise, deepdiff)
        → ContractDiffReport.build_report_data()  (transform to render dict)
        → TextContractDiffRenderer.render()  (plain-text output)
        → HtmlContractDiffRenderer.render()  (HTML output, style stripped)

Golden files:
    golden_integration_text.txt  — expected plain-text output
    golden_integration_html.html — expected HTML structure (style block replaced)
...
To regenerate golden files after an intentional output change:
    ...
    FIXTURE_DIR = "tests/fixtures/breaking/integration"
    ...
    rd = ContractDiffReport().build_report_data(
        ContractDiff().generate(V1, V2),
        source_label="v1.yaml", target_label="v2.yaml",
    )
    open(os.path.join(FIXTURE_DIR, "golden_integration_text.txt"), "w").write(
        TextContractDiffRenderer(report_data=rd).render()
    )
    html = HtmlContractDiffRenderer(report_data=rd).render()
    open(os.path.join(FIXTURE_DIR, "golden_integration_html.html"), "w").write(
        re.sub(r'<style>.*?</style>', '<style>...</style>', html, flags=re.DOTALL)
    )
    EOF
"""
...
FIXTURE_DIR = os.path.join(os.path.dirname(__file__), "fixtures", "breaking", "integration")
V1 = os.path.join(FIXTURE_DIR, "report_renderer_integration_v1.yaml")
V2 = os.path.join(FIXTURE_DIR, "report_renderer_integration_v2.yaml")
